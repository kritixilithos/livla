...
*/
    const benji = (source, socket, clientmensi, sendTo, what, action) => {
      if (source === "naxle") {
        socket.emit('la_livla_cu_cusku', {message: what});
        return what;
      } else {
        if (!action) {
    ...
    const GetXmldoc = (lng) => {
      if (lng !== "en" || !xmlDocEn) {
        ...
        return libxmljs.parseXml(fs.readFileSync(xmlPath, {encoding: 'utf8'}));
      }
      return xmlDocEn;
    };
 ...
 */
    ...
    const ningaumahantufa = (text, socket) => {
      ...
      try {
        ...
        socket.emit('la_livla_cu_cusku', {message: e.message});
      }
    };
    ...
    const updatexmldumps = callback => {
      ...
      langs.forEach(thisa => {
        ...
        request({uri, method: "GET", jar}).on("error", () => {
